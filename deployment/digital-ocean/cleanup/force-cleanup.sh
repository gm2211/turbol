...
# Cleanup pvcs except for vault
kubectl delete $(kubectl get pvc -o name | grep -v vault) || true
...
kubectl delete $(kubectl get -oname clusterroles | grep vault)
...
kubectl delete $(kubectl get -oname clusterrolebindings | grep vault)
...
kubectl delete $(kubectl get -oname MutatingWebhookConfiguration | grep vault)
kubectl delete $(kubectl get -oname ValidatingWebhookConfiguration | grep cert-manager)
kubectl delete $(kubectl get -oname ValidatingWebhookConfiguration | grep vault)
...
kubectl delete serviceaccount $(kubectl get serviceaccount | grep vault) || true
