...
def maybe_install_kubectl(cur_os):
  ...
  if cur_os == mac:
    ...
    bash("sudo curl -fsSLo /usr/share/keyrings/kubernetes-archive-keyring.gpg https://packages.cloud.google.com/apt/doc/apt-key.gpg")
    pipe(
      bash(
        "echo", 
...
def maybe_install_kubefwd(cur_os):
  ...
  if cur_os == mac:
    ...
    download_and_install_ubuntu("kubefwd.deb", "https://github.com/txn2/kubefwd/releases/download/1.22.0/kubefwd_{}.deb")
  elif cur_os == fedora:
    download_and_install_fedora("kubefwd.rpm", "https://github.com/txn2/kubefwd/releases/download/1.22.0/kubefwd_{}.rpm")
