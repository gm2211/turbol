...
def terraform_apply(cluster_name, script_dir, target = None):
  ...
  raw_bash('terraform apply {} --auto-approve -var="k8s_cluster_name={}" -var-file="{}/../secrets.tfvars"'.format(target_str, cluster_name, script_dir))
...
def terraform_populate_dev_vault(cluster_name, script_dir):
  terraform_apply(cluster_name, script_dir, "module.populate-dev-vault")
...
def start_all_services(cluster_name, old_cluster_was_deleted, script_dir):
  ...
  make_k8s_services_available_on_localhost() # make vault available on localhost
  terraform_populate_dev_vault(cluster_name, script_dir)
