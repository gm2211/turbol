...
object MoreSchedulers extends BackendLogging {
  ...
  private object ThreadFactoryBuilder {
    ...
    /** Constructs a ThreadFactory using the provided name prefix and appending with a unique incrementing thread
      ...
      * @param daemonic
      *   specifies whether the created threads should be daemonic (non-daemonic threads are blocking the JVM process on
      *   exit).
      */
    def apply(
        name: String,
    ...
    ): ThreadFactory = { (r: Runnable) =>
      {
        ...
        thread.setUncaughtExceptionHandler((_: Thread, e: Throwable) => reporter(e))
